...
make_DHelper(J) {
  ...
  //printf("decoding.jmp_eip = 0x%x ",decoding.jmp_eip);
  //printf("*eip = 0x%x ",*eip);
...
void operand_write(Operand *op, rtlreg_t* src) {
 printf("the op_write type is %d\n",op->type); 
  if (op->type == OP_TYPE_REG) { rtl_sr(op->reg, src, op->width); }
  else if (op->type == OP_TYPE_MEM) { rtl_sm(&op->addr, src, op->width);printf("the op_write type is memory\n"); }
  else { assert(0); }
}
